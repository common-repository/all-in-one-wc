...
if ( ! function_exists( 'aiow_maybe_add_european_union_countries' ) ) {
	 ...
	 * @return array
	 */
	function aiow_maybe_add_european_union_countries( $countries ) {
		if ( ! empty( $countries ) && in_array( 'EU', $countries ) ) {
			$countries = array_merge( $countries, aiow_get_european_union_countries() );
		}
		return $countries;
	}
}

if ( ! function_exists( 'aiow_get_country_by_ip' ) ) {
	/**
	 * Get country by IP.
	 *
	 * @return string
	 */
	function aiow_get_country_by_ip() {
		// Get the country by IP
		$location = ( class_exists( 'WC_Geolocation' ) ? WC_Geolocation::geolocate_ip() : array( 'country' => '' ) );
		// Base fallback
		if ( empty( $location['country'] ) ) {
			$location = apply_filters( 'woocommerce_customer_default_location', aiow_option( 'woocommerce_default_country' ) );
...
if ( ! function_exists( 'aiow_get_country_flag_by_code' ) ) {
	/**
	 ...
	 * @return string
	 */
	function aiow_get_country_flag_by_code( $country_code ) {
		$img      = '/assets/images/flag-icons/' . strtolower( $country_code ) . '.png';
		$img_path = aiow_plugin_path() . $img;
		$img_src  = aiow_plugin_url() . ( file_exists( $img_path ) ? $img : '/assets/images/flag-icons/no-flag.png' );
		return '<img src="' . $img_src . '" title="' . aiow_get_country_name_by_code( $country_code ) . '">';
	}
}

if ( ! function_exists( 'aiow_get_customer_country' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function aiow_get_customer_country( $user_id ) {
...
if ( ! function_exists( 'aiow_get_european_union_countries_with_vat' ) ) {
	/**
	 * Get european union countries with vat.
	 *
	 * @return array
	 */
	function aiow_get_european_union_countries_with_vat() {
		return array(
			'AT' => 20,
			'BE' => 21,
			'BG' => 20,
...
if ( ! function_exists( 'aiow_get_european_union_countries' ) ) {
	/**
	 * Get european union countries.
	 *
	 * @return array
	 */
	function aiow_get_european_union_countries() {
		return array_keys( aiow_get_european_union_countries_with_vat() );
	}
}

if ( ! function_exists( 'aiow_get_country_name_by_code' ) ) {
	/**
	 ...
	 */
	function aiow_get_country_name_by_code( $country_code ) {
		$countries = aiow_get_countries();
		return ( isset( $countries[ $country_code ] ) ) ? $countries[ $country_code ] : false;
	}
}

if ( ! function_exists( 'aiow_get_states' ) ) {
	/**
	 * Get all states array.
	 *
	 * @return  array
	 */
	function aiow_get_states() {
...
if ( ! function_exists( 'aiow_get_countries' ) ) {
	/**
	 * Get all countries array.
	 *
	 * @return  array
	 */
	function aiow_get_countries() {
		return array(
...
if ( ! function_exists( 'aiow_get_currency_countries' ) ) {
	/**
	 * Get currency countries.
	 *
	 * @return array
	 */
	function aiow_get_currency_countries() {
		return array(
			'AFN' => array( 'AF' ),
			'ALL' => array( 'AL' ),
			'DZD' => array( 'DZ' ),
