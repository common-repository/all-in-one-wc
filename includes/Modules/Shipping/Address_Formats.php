<?php
/**
 * Shipping Module - Address formats.
 *
 * @package WordPress
...
if ( ! class_exists( 'Address_Formats' ) ) {

	/**
	 * Declare class `Address_Formats` extends to `\AIOW\Modules\Register_Modules`.
	 */
	class Address_Formats extends \AIOW\Modules\Register_Modules {
		...
		function __construct() {
			...
			$this->short_desc = __( 'Address Formats', 'all-in-one-wc' );
			$this->desc       = __( 'Set address format in orders on per country basis. Force base country display.', 'all-in-one-wc' );
		 ...
		 * Display country.
		 *
		 * @param string $display Country display.
		 * @return string
		 */
		function customize_force_country_display( $display ) {
			return ( 'yes' === aiow_option( 'aiow_address_formats_force_country_display', 'no' ) );
		}

		/**
		 * Customize address formats.
		 *
		 * @param array $formats Format array.
		 * @return array
		 */
		function customize_address_formats( $formats ) {
			...
			foreach ( $default_formats as $country_code => $format ) {
				...
				$format = aiow_option( 'aiow_address_formats_country_' . $country_code, $default_format );
		...
		function get_default_address_formats() {
			...
			$all_countries = aiow_get_countries();
