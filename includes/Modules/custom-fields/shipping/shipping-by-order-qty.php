<?php
/**
 * Shipping - Settings - Shipping Methods by Min/Max Order Quantity
 *
 * @package WordPress
 * @subpackage WooCommerce
 */

$use_shipping_instances = ( 'yes' === aiow_option( 'aiow_shipping_by_order_qty_use_shipping_instance', 'no' ) );
$settings = array(
	array(
		'title'    => __( 'General Options', 'all-in-one-wc' ),
		'type'     => 'title',
		'id'       => 'aiow_shipping_by_order_qty_general_options',
	),
	array(
		...
		'type'     => 'checkbox',
		'id'       => 'aiow_shipping_by_order_qty_use_shipping_instance',
		'default'  => 'no',
	),
	array(
		'type'     => 'sectionend',
	...
	array(
		'title'   => __( 'Shipping Methods by Min/Max Order Quantity', 'all-in-one-wc' ),
		'type'    => 'title',
		'desc'    => __( 'Set to zero to disable.', 'all-in-one-wc' ),
		'id'      => 'aiow_shipping_by_order_qty_options',
	),
);
...
foreach ( $shipping_methods as $method ) {
	...
	$settings = array_merge( $settings, array(
		array(
			...
			'desc_tip'  => $desc_tip,
			'desc'      => '<br>' . __( 'Minimum order quantity', 'all-in-one-wc' ),
			'id'        => 'aiow_shipping_by_order_qty_min' . ( $use_shipping_instances ? '_instance[' . $method['shipping_method_instance_id'] . ']' : '[' . $method->id . ']' ),
			'default'   => 0,
			'type'      => 'number',
		...
		array(
			'desc_tip'  => $desc_tip,
			'desc'      => '<br>' . __( 'Maximum order quantity', 'all-in-one-wc' ),
			'id'        => 'aiow_shipping_by_order_qty_max' . ( $use_shipping_instances ? '_instance[' . $method['shipping_method_instance_id'] . ']' : '[' . $method->id . ']' ),
			'default'   => 0,
			'type'      => 'number',
			'custom_attributes' => $custom_attributes,
		),
